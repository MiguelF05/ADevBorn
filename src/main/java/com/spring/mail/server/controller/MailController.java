package com.spring.mail.server.controller;

import com.spring.mail.server.domain.EmailDTO;
import com.spring.mail.server.domain.EmailFileDTO;
import com.spring.mail.server.service.IEmailService;
...
public class MailController {
    ...
    /*@PostMapping("/sendMessage")
    public ResponseEntity<?> receiveRequestEmail(@RequestBody EmailDTO emailDTO){
        System.out.println("Mensage recibido"+emailDTO);
        emailService.sendEmail(emailDTO.getToUser(),emailDTO.getSubject(),emailDTO.getMessage());
        Map<String,String>response =new HashMap<>();
        response.put("estado","Enviado");
        return ResponseEntity.ok(response);
